...
//	@HystrixCommand(fallbackMethod= "defaultResponse")
  ...
  //------------------------------------------------------------------------------------------------------------//	
  //------------------------------------------- Spring Employee Client -----------------------------------------//	
  //------------------------------------------------------------------------------------------------------------//	
	@GetMapping(value="/all/getAllEmployees", produces= MediaType.APPLICATION_JSON_VALUE)
	//@HystrixCommand(fallbackMethod= "defaultResponse")
	public ResponseEntity<ArrayList> getEmployeesAndUserInfoViaFeign() {
		System.out.println("Using the feign client controller to fetch the employees information from Database DB");
 
		// Fetching the all employees information from the DB.
		// Data is fetched from thr employees microservice hosted on port no. - 8282
